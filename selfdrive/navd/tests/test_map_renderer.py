...
class TestMapRenderer:
  ...
  def _run_test(self, expect_valid, locations=LOCATION1_REPEATED):
    ...
    for i, location in enumerate(locations):
      ...
      live_pose = generate_livePose(location)
      self.pm.send("livePose", live_pose)
      ...
      assert self.sm['mapRenderState'].locationMonoTime == live_pose.logMonoTime
      if not expect_valid:
      ...
      assert self.vipc.timestamp_sof == live_pose.logMonoTime
