...
class SpeedLimitAssist:
  ...
  def update_active_event(self, events: Events) -> None:
    if self.v_cruise_cluster_below_confirm_speed_threshold:
      events.add(EventName.speedLimitChanged)
    else:
  ...
  def update_events(self, events: Events) -> None:
    if self.state == SpeedLimitAssistState.preActive:
    ...
    if self.state == SpeedLimitAssistState.pending and self._state_prev != SpeedLimitAssistState.pending:
      events.add(EventName.speedLimitPending)

    if self.is_active:
      if self._state_prev not in ACTIVE_STATES:
        self.update_active_event(events)
  ...
  def update(self, long_enabled: bool, long_override: bool, v_ego: float, a_ego: float, v_cruise_cluster: float, speed_limit: float,
             speed_limit_final_last: float, has_speed_limit: bool, distance: float, events: Events) -> None:
