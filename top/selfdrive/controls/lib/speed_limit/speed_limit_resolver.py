...
from openpilot.top import PARAMS_UPDATE_PERIOD
from openpilot.top.selfdrive.controls.lib.speed_limit import LIMIT_MAX_MAP_DATA_AGE, LIMIT_ADAPT_ACC
from openpilot.top.selfdrive.controls.lib.speed_limit.common import Policy, OffsetType

SpeedLimitSource = custom.LongitudinalPlanTOP.SpeedLimit.Source

ALL_SOURCES = tuple(SpeedLimitSource.schema.enumerants.values())


class SpeedLimitResolver:
  limit_solutions: dict[custom.LongitudinalPlanTOP.SpeedLimit.Source, float]
  distance_solutions: dict[custom.LongitudinalPlanTOP.SpeedLimit.Source, float]
  v_ego: float
  speed_limit: float
  ...
  distance: float
  source: custom.LongitudinalPlanTOP.SpeedLimit.Source
  ...
  def _reset_limit_sources(self, source: custom.LongitudinalPlanTOP.SpeedLimit.Source) -> None:
  ...
  def _get_from_car_state(self, sm: messaging.SubMaster) -> None:
    ...
    self.limit_solutions[SpeedLimitSource.car] = sm['carStateTOP'].speedLimit
  ...
  def _get_source_solution_according_to_policy(self) -> custom.LongitudinalPlanTOP.SpeedLimit.Source:
  ...
  def _resolve_limit_sources(self, sm: messaging.SubMaster) -> tuple[float, float, custom.LongitudinalPlanTOP.SpeedLimit.Source]:
