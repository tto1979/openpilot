...
from openpilot.top.selfdrive.controls.lib.speed_limit.common import Mode
from openpilot.top.selfdrive.controls.lib.speed_limit import PCM_LONG_REQUIRED_MAX_SET_SPEED
...
class TestSpeedLimitAssist:
  ...
  def test_disabled(self):
    ...
    for _ in range(int(10. / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, SPEED_LIMITS['highway'], SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events)
    assert self.sla.state == SpeedLimitAssistState.disabled
  ...
  def test_transition_disabled_to_pending_no_speed_limit_not_max_initial_set_speed(self):
    for _ in range(int(3. / DT_MDL)):
      self.sla.update(True, False, SPEED_LIMITS['highway'], 0, SPEED_LIMITS['city'], 0, 0, False, 0, self.events)
  ...
  def test_preactive_to_active_with_max_speed_confirmation(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['highway'],
                    SPEED_LIMITS['highway'], True, 0, self.events)
  ...
  def test_preactive_to_pending_no_speed_limit(self):
    self.sla.state = SpeedLimitAssistState.preActive
    self.sla.update(True, False, SPEED_LIMITS['highway'], 0, self.pcm_long_max_set_speed, 0, 0, False, 0, self.events)
    assert self.sla.state == SpeedLimitAssistState.pending
    assert self.sla.is_enabled and not self.sla.is_active
  ...
  def test_pending_to_active_when_speed_limit_available(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['highway'], 0, self.pcm_long_max_set_speed,
                    SPEED_LIMITS['highway'], SPEED_LIMITS['highway'], True, 0, self.events)
    assert self.sla.state == SpeedLimitAssistState.active
  ...
  def test_manual_cruise_change_detection(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, different_cruise, SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events)
    assert self.sla.state == SpeedLimitAssistState.inactive

  # TODO-TOP: test lower CST cases
  def test_rapid_speed_limit_changes(self):
    ...
    for _, speed_limit in enumerate(speed_limits):
      self.sla.update(True, False, speed_limit, 0, self.pcm_long_max_set_speed, speed_limit, speed_limit, True, 0, self.events)
  ...
  def test_invalid_speed_limits_handling(self):
    ...
    for invalid_limit in invalid_limits:
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, invalid_limit, SPEED_LIMITS['city'], True, 0, self.events)
  ...
  def test_stale_data_handling(self):
    ...
    self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, 0, old_speed_limit, True, 0, self.events)
    assert self.sla.state in ACTIVE_STATES
  ...
  def test_distance_based_adapting(self):
    ...
    self.sla.update(True, False, current_speed, 0, self.pcm_long_max_set_speed, target_speed, target_speed, True, distance, self.events)
    assert self.sla.state == SpeedLimitAssistState.adapting
    assert self.sla.output_v_target == target_speed  # TODO-TOP: assert expected accel, need to enable self.acceleration_solutions
  ...
  def test_long_disengaged_to_disabled(self):
    ...
    self.sla.update(False, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'],
                    SPEED_LIMITS['city'], True, 0, self.events)
    assert self.sla.state == SpeedLimitAssistState.disabled
  ...
  def test_maintain_states_with_no_changes(self):
    ...
    for state in test_states:
      ...
      self.sla.update(True, False, SPEED_LIMITS['city'], 0, self.pcm_long_max_set_speed, SPEED_LIMITS['city'], SPEED_LIMITS['city'], True, 0, self.events)
