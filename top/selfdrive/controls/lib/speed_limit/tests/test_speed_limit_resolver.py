...
from openpilot.top.selfdrive.controls.lib.speed_limit import LIMIT_MAX_MAP_DATA_AGE

from openpilot.top.selfdrive.controls.lib.speed_limit.speed_limit_resolver import SpeedLimitResolver, ALL_SOURCES
from openpilot.top.selfdrive.controls.lib.speed_limit.common import Policy
...
def setup_sm_mock(mocker: MockerFixture):
  ...
  sm_mock.__getitem__.side_effect = lambda key: {
    'carState': car_state,
    'liveMapDataTOP': live_map_data,
    'carStateTOP': car_state_top,
...
parametrized_policies = pytest.mark.parametrize(
  "policy, sm_key, function_key", [
    (Policy.car_state_only, 'carStateTOP', SpeedLimitSource.car),
    (Policy.car_state_priority, 'carStateTOP', SpeedLimitSource.car),
    (Policy.map_data_only, 'liveMapDataTOP', SpeedLimitSource.map),
    (Policy.map_data_priority, 'liveMapDataTOP', SpeedLimitSource.map),
  ],
...
class TestSpeedLimitResolverValidation:
  ...
  def test_resolver_combined(self, resolver_class, mocker: MockerFixture):
    ...
    sm_mock = setup_sm_mock(mocker)
    socket_to_source = {'carStateTOP': SpeedLimitSource.car, 'liveMapDataTOP': SpeedLimitSource.map}
